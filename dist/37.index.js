...
/* harmony export */   declarationDigest: () => (/* binding */ declarationDigest),
 ...
 */
async function keyFingerprint(publicKeyBase64) {
 ...
 */
function parseProvenanceId(provenanceId) {
 ...
 * }>}
 */
async function verifyDeclaration(declaration, options = {}) {
  const base = {
    signed: false,
    valid: false,
    reason: null,
  ...
  try {
    payload =
      coverage === 'declaration'
        ? (0,_canonical_js__WEBPACK_IMPORTED_MODULE_0__/* .declarationSigningPayload */ .aK)(declaration)
 ...
 */
async function verifyAgentChallenge(publicKeyBase64, provenanceId, nonce, signatureBase64) {
  try {
 ...
 * @returns {Promise<boolean>}
 */
async function verifyAgentRevocation(publicKeyBase64, provenanceId, signatureBase64) {
  try {
 ...
 * @returns {Promise<boolean>}
 */
async function verifyChallenge(publicKeyBase64, provenanceId, nonce, signatureBase64) {
  try {
...
async function declarationDigest(declaration) {
  const bytes = new TextEncoder().encode((0,_canonical_js__WEBPACK_IMPORTED_MODULE_0__/* .declarationSigningPayload */ .aK)(declaration));
 ...
 * }>}
 */
async function verifyNotice(notice, options = {}) {
  const out = (status, reason, extra = {}) => ({
 ...
 */
function checkInteropLinks(declaration) {
